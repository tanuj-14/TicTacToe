...
# Check if the game has been won by either player or computer
def gameWon(board):
...
# Get the list of blank cells on the board
def blanks(board):
    blank = []
    for x, row in enumerate(board):
        for y, col in enumerate(row):
...
# Get user input for a player's move
...
# Get the score of the current board state
def getScore(board):
    if winningPlayer(board, 1):
        return 10
    elif winningPlayer(board, -1):
...
# Player vs. Computer (PvC) game loop
def pvc():
    while True:
        try:
    ...
    # Clear the board and set the current player based on the order
    Clearboard(board)
    if order == 2:
        currentPlayer = -1
    else:
